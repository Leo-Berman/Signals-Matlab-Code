% file: e9_3_LV9_1_LP.m  % brickwall discrete lowpass filter, Hamming windowed
clear; close all; set(0,'defaultAxesFontSize',13);
L = 10;      % impulse response is of length 2*L+1, noncausal
fc = 200;    % corner (cutoff) frequency (actually is where |H(f)|=0.5, not 0.707)
...
subplot(2,1,1), plot(f,FH),grid minor,title('Frequency response |H(f)|'),xlabel('f (Hz)')
